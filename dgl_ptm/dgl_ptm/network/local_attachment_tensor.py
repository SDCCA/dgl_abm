...
def local_attachment_tensor(graph,n_FoF_links,edge_prop=None,p_attach=1.):
    ...
    selected_links_matrix = matrix_utils.apply_mask_to_sparse_matrix(adj_matrix, 
                                                                     selected_links)
    FoF_field_matrix, FoF_field_matrix_norm_eprop = neighbour_field_matrix(
                                                                selected_links_matrix,
                                                                adj_matrix)
    new_FoF = FoF_field_matrix.val > 0
    new_FoF_norm_eprop = matrix_utils.apply_mask_to_sparse_matrix(
    ...
    if torch.count_nonzero(new_FoF) <= n_FoF_links:
        ...
        selected_FoF = new_FoF_renorm.flatten().multinomial(num_samples=n_FoF_links,
                                                            replacement=False)
        #probe_p_attach = torch.rand(new_FoF_norm_eprop.val.shape[0])
    ...
    FoF_to_link_matrix = matrix_utils.apply_mask_to_sparse_matrix(new_FoF_norm_eprop, 
                                                                  to_link)
