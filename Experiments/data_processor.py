...
def save_histogram_data(dfs, labels, filename, column="wealth"):
    ...
    for df, label in zip(dfs, labels):
        ...
        for bin_edge, percent_count in zip(bin_edges, percent_counts):
            all_data.append({"Arrangement": label, "bin": bin_edge, "frequency": percent_count})
...
# def store_seed_histogram_data(dfs, labels, column="wealth"):
#     all_data = []
#     for df, label in zip(dfs, labels):
#         for seed in completed_seeds:
#             for time_step in df['time_step'].unique():
#                 seed_df = df[(df['seed'] == seed) & (df['time_step'] == time_step)]
#                 bins = np.arange(0, seed_df[column].max() + 1, 1)
#                 bin_edges, counts = calculate_histogram_data(seed_df, column, bins)
#                 for bin_edge, count in zip(bin_edges, counts):
#                     all_data.append({"Arrangement": label, "seed": seed, "time_step": time_step, "bin": bin_edge, "count": count})
