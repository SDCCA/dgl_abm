...
def apply_mask_to_sparse_matrix(sparse_matrix: SparseMatrix, mask: torch.Tensor) -> SparseMatrix:
    ...
    """
    return dgl.sparse.from_coo(
        sparse_matrix.row[mask], sparse_matrix.col[mask], sparse_matrix.val[mask], sparse_matrix.shape
    )


def downstream_nodes(source_id: int, destination_id: int, graph: dgl.DGLGraph) -> torch.Tensor:
    ...
    return successors_[successors_ != source_id]


def existing_connections(source_id: int, destination_ids: int | Iterable[int], graph: dgl.DGLGraph) -> torch.Tensor:
    ...
    """
    return graph.has_edges_between(source_id, destination_ids)


def sparse_matrix_to_triangular(sparse_matrix: SparseMatrix, selection: str | None = None) -> SparseMatrix:
    """Select either the upper or lower triangular matrix from a sparse matrix.
    ...
    if selection == "upper":
        ...
        message = 'Triangular matrix not specified (must be "upper" or "lower")'
        raise ValueError(message)
    return apply_mask_to_sparse_matrix(sparse_matrix, mask)


def sparse_matrix_to_upper_triangular(sparse_matrix: SparseMatrix) -> SparseMatrix:
...
def sparse_matrix_to_lower_triangular(sparse_matrix: SparseMatrix) -> SparseMatrix:
...
def upper_triangular_to_symmetrical(triangular: SparseMatrix) -> SparseMatrix:
...
def clear_symmetrical_duplicates(sparse_matrix: SparseMatrix) -> tuple[SparseMatrix, SparseMatrix]:
    """Remove possible duplicate edges.

    Assuming a matrix (nxn) with non-zero elements spread throughout both upper
    lower triangular sub-matrices is a sub-selection of a symmetrical matrix,
