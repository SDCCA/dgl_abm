...
def average_degree(graph: DGLGraph) -> float:
...
def average_weighted_degree(graph: DGLGraph) -> float:
    ...
    """
    return (2 * torch.sum(graph.edata["weight"]) / graph.number_of_nodes()).item()


def node_degree(graph: DGLGraph) -> torch.Tensor:
...
def node_weighted_degree(graph: DGLGraph) -> torch.Tensor:
    """Compute the weighted degree of each node in the graph.

    Note: The result is in and out combined.

    param: graph: dgl_ptm graph
    return: torch.Tensor: The weighted degree of each node.
    """
    weights_sparse = sparse.spmatrix(
        torch.stack(graph.edges(order="eid"), dim=0),
        graph.edata["weight"].to(torch.float32),
        shape=(graph.number_of_nodes(), graph.number_of_nodes()),
    )
    ones = torch.ones(graph.number_of_nodes(), device=graph.device, dtype=torch.float32)

    return sparse.matmul(weights_sparse, ones) + sparse.matmul(weights_sparse.T, ones)
