...
def network_creation(num_agents: int, method: str, **kwargs: dict) -> dgl.DGLGraph:
    ...
    if (method == "barabasi-albert"):
        ...
        agent_graph = edgeless_graph(num_agents)
    else:
        message = "Currently only barabasi-albert network creation is supported."
...
def edgeless_graph(num_agents: int) -> dgl.DGLGraph:
    ...
    Returns:
        agent_graph (DGLGraph): DGL graph with nodes and no edges
    """
    return dgl.graph(([], []), num_nodes=num_agents)
