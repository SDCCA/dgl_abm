...
def link_deletion(agent_graph, method: str, threshold: float):
    ...
    """
    agent_graph.remove_edges(_select_edges(agent_graph, method = method, 
                                           threshold = threshold))
...
def _select_edges(agent_graph, method: str, threshold: float):
    ...
    if method == "probability":
        ...
        mask_edges = (((1.-agent_graph.edata['weight']) * 
                      torch.rand(upper_triangular.val.size()[0])) < threshold)
    elif method == "size":
        ...
        raise NotImplementedError('Currently only "probability" and "size" deletion '
                                  'methods are supported')
        mask_edges = torch.zeros(upper_triangular.val.size()[0])

    deletion_matrix_upper_tri = matrix_utils.apply_mask_to_sparse_matrix(
                                                        upper_triangular, mask_edges)
    deletion_matrix = matrix_utils.upper_triangular_to_symmetrical(
                                                        deletion_matrix_upper_tri)
